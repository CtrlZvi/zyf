...
const addCustomAccountValidations = <T extends z.ZodRawShape>(
    object: z.ZodObject<T>,
) =>
    object
        .refine(
            (account) =>
                !["merchant", "person"].includes(account.type) ||
                account.institution === undefined ||
                account.institution === null,
            {
                message: "Merchants and people are not financial institutions",
                path: ["institution"],
            },
        )
        .refine(
            (account) =>
                ...
                (account.initialBalanceDate === undefined ||
                    account.initialBalanceDate === null),
            {
                message: "An initial balance must have a date",
                path: ["initialBalanceDate"],
            },
        )
        .refine(
            (account) =>
                ...
                !account.internal,
            {
                message: "Merchants and people must be external accounts",
                path: ["internal"],
            },
        );
...
export const CreateAccount = addCustomAccountValidations(
    AccountBase.omit({ id: true }),
);
...
export const accounts = sqliteTable(
    "accounts",
    {
        ...
        initialBalanceDate: integer("initial_balance_date", {
            mode: "timestamp",
        }),
    ...
    (table) => ({
        ...
        uniqueAccountNumber: uniqueIndex("account_number_idx").on(
            table.name,
            table.type,
        ),
    }),
