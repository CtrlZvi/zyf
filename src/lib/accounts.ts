...
import { createAccount as createAccountInDatabase, fetchAccounts as fetchAccountsFromDatabase } from './database';

export async function createAccount(prevState: accountSchema.CreateAccountParseError, formData: FormData) {
    console.debug(`Form data:`);
    console.debug(Object.fromEntries(formData.entries()));
    const account = accountSchema.CreateAccount.safeParse(Object.fromEntries(formData.entries()));
    ...
    if (account.data.url) {
        try {
            ...
            const response = await fetch(`https://icons.duckduckgo.com/ip3/${url.hostname}.ico`);
            if (response.ok) {
            ...
            console.warn(`Could not get favicon for ${account.data.name}: ${error}`);
        }
    }
...
export async function fetchAccounts() {
    return (await fetchAccountsFromDatabase()).map((account) => accountSchema.Account.parse(account));
}